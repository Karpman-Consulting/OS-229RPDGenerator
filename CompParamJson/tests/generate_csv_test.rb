...
class GenerateCsvDataTest < Minitest::Test
  ...
  def test_generate_comp_param_empty_csv
    ...
    csv_data = GenerateCompParamJsonCsv.produce_csv_data_from_comp_param_json(@empty_cp_json)
  ...
  def test_comp_param_json_to_csv_data
    ...
    csv_row_of_perimeter_zn_one_ag_factor = csv_data.find { |csv_row_data| csv_row_data[:group_id].downcase == "PERIMETER_ZN_1".downcase &&
    ...
    csv_row_of_psz_ac_3 = csv_data.select { |csv_row_data| csv_row_data[:group_id] == "PSZ-AC:3" &&
    ...
    csv_row_of_psz_ac_3_fan = csv_data.find { |csv_row_data| csv_row_data[:group_id] == "PSZ-AC:3 FAN-fansystem" &&
    ...
    csv_row_of_zn_two_wall_east_window_four_framing_type = csv_data.find { |csv_row_data| csv_row_data[:group_id].downcase == "PERIMETER_ZN_2_WALL_EAST_WINDOW_4".downcase &&
    ...
    ### Run the code
    updated_cp_json = GenerateCompParamJsonCsv.set_comp_param_json_from_csv_data(@empty_cp_json,csv_data)
    ...
    csv_row_of_perimeter_zn_one_updated = GenerateCompParamJsonCsv.find_by_id(updated_cp_json, "PERIMETER_ZN_1".downcase)

    assert_equal new_zone_aggregation_factor, csv_row_of_perimeter_zn_one_updated["aggregation_factor"]

    csv_row_of_psz_ac_3_updated = GenerateCompParamJsonCsv.find_by_id(updated_cp_json, "PSZ-AC:3")

    assert_equal new_status_type_two, csv_row_of_psz_ac_3_updated["status_type"]

    csv_row_of_asz_ac_fan = GenerateCompParamJsonCsv.find_by_id(updated_cp_json, "PSZ-AC:3 FAN-fansystem")

    assert_equal 'new_fan_system_air_filter_merv_rating', csv_row_of_asz_ac_fan['air_filter_merv_rating']

    zn_two_wall_east_window_four = GenerateCompParamJsonCsv.find_by_id(updated_cp_json, "PERIMETER_ZN_2_WALL_EAST_WINDOW_4".downcase)
  ...
  def test_comp_param_json_to_csv_data_e1
    ...
    csv_data = GenerateCompParamJsonCsv.produce_csv_data_from_comp_param_json(@empty_cp_json_e1)
