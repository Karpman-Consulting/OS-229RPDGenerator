module SetBuildingSegements
  ...
  def self.read_csv_and_set_building_segments_in_comp_param_json(csv_data,comp_param_json)
    ...
    unique_building_segments_csv_ids = csv_data.select { |row| row[:compliance_parameter_category] == "building_segment" }.map { |row| row[:two_twenty_nine_group_id] }.uniq

    child_objects_of_building_segments_csv = csv_data.select { |row| unique_building_segments_csv_ids.include? row[:two_twenty_nine_parent_id] } #.select { |row| row[:compliance_parameter_category] == "building_segment" }.map { |row| row[:two_twenty_nine_group_id] }.uniq
    ...
    unless building_segment_ids_json.sort == unique_building_segments_csv_ids.sort
      ...
      new_building_segments_in_csv.each do |new_building_segment_id|
        ...
        new_building_segment_compliance_parameters = csv_data.select { |row| row[:two_twenty_nine_group_id] == new_building_segment_id }
        ...
        child_objs_of_new_building_segment_in_csv = child_objects_of_building_segments_csv.select { |child_object| child_object[:two_twenty_nine_parent_id] == new_building_segment_id }
        ...
        building_segments.map do |building_segment|
          building_segment["heating_ventilating_air_conditioning_systems"].each do |hvac_system|
            ...
            if child_objs_of_new_building_segment_in_csv.map { |obj| obj[:two_twenty_nine_group_id]}.include? hvac_system.dig("id")
        ...
        building_segments.map do |building_segment|
          building_segment["zones"].each do |zone|
            if child_objs_of_new_building_segment_in_csv.map { |obj| obj[:two_twenty_nine_group_id]}.include? zone.dig("id")
              ## Move hvac system to new building segment
